...
type Runtime struct {
	*escapeeWriter
	*scope
	content func(*Runtime, Expression) errors.Error
...
func (rt *Runtime) YieldBlock(name string, context interface{}) errors.Error {
...
func (rt *Runtime) setValue(name string, val reflect.Value) errors.Error {
...
func (rt *Runtime) resolve(name string) (reflect.Value, errors.Error) {
	if name == "." {
...
func (rt *Runtime) executeSet(left Expression, right reflect.Value) errors.Error {
	typ := left.Type()
...
func (rt *Runtime) executeSetList(set *SetNode) errors.Error {
...
func (rt *Runtime) executeLetList(set *SetNode) errors.Error {
	if set.IndexExprGetLookup {
		value, err := rt.evalPrimaryExpressionGroup(set.Right[0])
		if err != nil {
			return err
...
func (rt *Runtime) executeYieldBlock(block *BlockNode, blockParam, yieldParam *BlockParameterList, expression Expression, content *ListNode) errors.Error {
	...
	if content != nil {
		myscope := rt.scope
		rt.content = func(st *Runtime, expression Expression) errors.Error {
			outscope := st.scope
...
func (rt *Runtime) executeList(list *ListNode) (returnValue reflect.Value, err errors.Error) {
...
func (rt *Runtime) executeTry(try *TryNode) (returnValue reflect.Value, err errors.Error) {
	writer := rt.Writer
...
func (rt *Runtime) executeInclude(node *IncludeNode) (returnValue reflect.Value, err errors.Error) {
	...
	if !name.IsValid() {
		return reflect.Value{}, node.error(errors.InvalidValueReason, "evaluating name of template to include: name is not a valid value")
	}
	if name.Type().Implements(stringerType) {
		...
		return reflect.Value{}, node.error(errors.UnexpectedExpressionTypeReason, fmt.Sprintf("evaluating name of template to include: unexpected expression type %q", getTypeString(name)))
	}
...
func (rt *Runtime) evalPrimaryExpressionGroup(node Expression) (reflect.Value, errors.Error) {
	switch node.Type() {
	...
	case NodeSliceExpr:
		...
		if node.Index != nil {
			...
			if canNumber(indexExpression.Kind()) {
				...
				return reflect.Value{}, node.Index.error(errors.InvalidValueReason, fmt.Sprintf("non numeric value in index expression kind %s", indexExpression.Kind().String()))
			}
		}
		...
		if node.EndIndex != nil {
			...
			if canNumber(indexExpression.Kind()) {
				...
				return reflect.Value{}, node.EndIndex.error(errors.InvalidValueReason, fmt.Sprintf("non numeric value in index expression kind %s", indexExpression.Kind().String()))
			}
		} else {
...
func (rt *Runtime) isSet(node Node) (ok bool, err errors.Error) {
	defer func() {
		if r := recover(); r != nil {
	...
	default:
		...
		if !(nodeType > beginExpressions && nodeType < endExpressions) {
			return false, node.error(errors.UnexpectedNodeReason, fmt.Sprintf("unexpected %q node in isset clause", node))
		}
	}
	return true, nil
}

func (rt *Runtime) evalNumericComparativeExpression(node *NumericComparativeExprNode) (reflect.Value, errors.Error) {
	...
	case itemGreat:
		if isInt(kind) {
			...
			return reflect.Value{}, node.Left.error(errors.InvalidValueReason, "a non numeric value in numeric comparative expression")
		}
...
func (rt *Runtime) evalLogicalExpression(node *LogicalExprNode) (reflect.Value, errors.Error) {
	left, err := rt.evalPrimaryExpressionGroup(node.Left)
	if err != nil {
		return reflect.Value{}, err
	}
...
func (rt *Runtime) evalComparativeExpression(node *ComparativeExprNode) (reflect.Value, errors.Error) {
	left, err := rt.evalPrimaryExpressionGroup(node.Left)
	if err != nil {
		return reflect.Value{}, err
	}
...
func toInt(v reflect.Value) int64 {
	if !v.IsValid() {
		panic(errors.InvalidValueErr.WithMessage("invalid value can't be converted to int64"))
	}
	kind := v.Kind()
	if isInt(kind) {
		return v.Int()
		...
		if err != nil {
			panic(errors.New().WithReason("invalid.parse").WithMessage(err.Error()))
		}
		return n
	...
	panic(errors.New().WithReason("invalid.type").
...
func toUint(v reflect.Value) uint64 {
	if !v.IsValid() {
		panic(errors.InvalidValueErr.WithMessage("invalid value can't be converted to uint64"))
	}
	kind := v.Kind()
	if isUint(kind) {
		return v.Uint()
...
func toFloat(v reflect.Value) float64 {
	if !v.IsValid() {
		panic(errors.InvalidValueErr.WithMessage("invalid value can't be converted to float64"))
	}
	kind := v.Kind()
	if isFloat(kind) {
		return v.Float()
...
func (rt *Runtime) evalMultiplicativeExpression(node *MultiplicativeExprNode) (reflect.Value, errors.Error) {
	left, err := rt.evalPrimaryExpressionGroup(node.Left)
	if err != nil {
		return reflect.Value{}, err
	}
	...
	case itemMul:
		if isInt(kind) {
			...
			return reflect.Value{}, node.Left.error(errors.InvalidValueReason, "a non numeric value in multiplicative expression")
		}
	case itemDiv:
		if isInt(kind) {
			if needFloatPromotion {
...
func (rt *Runtime) evalAdditiveExpression(node *AdditiveExprNode) (reflect.Value, errors.Error) {
	...
	if node.Left == nil {
		...
		if !right.IsValid() {
			return reflect.Value{}, node.error(errors.InvalidValueReason, "right side of additive expression is invalid value")
		}
		kind := right.Kind()
		// todo: optimize
		if isInt(kind) {
		...
		return reflect.Value{}, node.Left.error(errors.InvalidValueReason, fmt.Sprintf("additive expression: right side %s (%s) is not a numeric value (no left side)", node.Right, getTypeString(right)))
	}
	...
	if !left.IsValid() {
		return reflect.Value{}, node.error(errors.InvalidValueReason, "left side of additive expression is invalid value")
	}
	if !right.IsValid() {
		return reflect.Value{}, node.error(errors.InvalidValueReason, "right side of additive expression is invalid value")
	}
	...
	if needFloatPromotion {
		if isInt(kind) {
			...
			return reflect.Value{}, node.Left.error(errors.InvalidValueReason, fmt.Sprintf("additive expression: left side (%s (%s) needs float promotion but neither int nor uint)", node.Left, getTypeString(left)))
		}
	} else {
		if isInt(kind) {
			if isAdditive {
			...
			return reflect.Value{}, node.Left.error(errors.InvalidValueReason, fmt.Sprintf("additive expression: left side %s (%s) is not a numeric value", node.Left, getTypeString(left)))
		}
	}
...
func (rt *Runtime) evalBaseExpressionGroup(node Node) (reflect.Value, errors.Error) {
	switch node.Type() {
	case NodeNil:
	...
	case NodeField:
		...
		for i := 0; i < len(node.Idents); i++ {
			...
			if !field.IsValid() {
				return reflect.Value{}, node.error(errors.NotFoundFieldOrMethodReason, fmt.Sprintf("there is no field or method '%s' in %s (.%s)", node.Idents[i].name, getTypeString(resolved), strings.Join(node.Idents.names(), ".")))
			}
			resolved = field
		}
		return resolved, nil
	...
	return reflect.Value{}, node.error(errors.UnexpectedNodeTypeReason, fmt.Sprintf("unexpected node type %s in unary expression evaluating", node))
}

func (rt *Runtime) evalCallExpression(baseExpr reflect.Value, args CallArgs) (reflect.Value, errors.Error) {
	return rt.evalPipeCallExpression(baseExpr, args, nil)
}

func (rt *Runtime) evalPipeCallExpression(baseExpr reflect.Value, args CallArgs, pipedArg *reflect.Value) (reflect.Value, errors.Error) {
	if !baseExpr.IsValid() {
		return reflect.Value{}, errors.New().
...
func (rt *Runtime) evalCommandExpression(node *CommandNode) (reflect.Value, bool, errors.Error) {
...
func (rt *Runtime) evalChainNodeExpression(node *ChainNode) (reflect.Value, errors.Error) {
...
func (rt *Runtime) evalSafeWriter(term reflect.Value, node *CommandNode, v ...reflect.Value) errors.Error {
	sw := &escapeWriter{rawWriter: rt.Writer, safeWriter: term.Interface().(SafeWriter)}
	for i := 0; i < len(v); i++ {
		if _, err := fastprinter.PrintValue(sw, v[i]); err != nil {
			return errors.New().WithReason("invalid.arguments").WithMessage(err.Error())
		}
	}
...
func (rt *Runtime) evalCommandPipeExpression(node *CommandNode, value reflect.Value) (reflect.Value, bool, errors.Error) {
	term, err := rt.evalPrimaryExpressionGroup(node.BaseExpr)
	if err != nil {
		return reflect.Value{}, false, err
	}
	if !term.IsValid() {
		return reflect.Value{}, false, node.error(errors.InvalidValueReason, "base expression of command pipe node is invalid value")
	}
	if term.Kind() != reflect.Func {
...
func (rt *Runtime) evalPipelineExpression(node *PipeNode) (value reflect.Value, safeWriter bool, err errors.Error) {
	...
	for i := 1; i < len(node.Cmds); i++ {
		if safeWriter {
			return reflect.Value{}, false, node.Cmds[i].error(errors.UnexpectedCommandReason, fmt.Sprintf("unexpected command %s, writer command should be the last command", node.Cmds[i]))
		}
...
func (rt *Runtime) evaluateArgs(fnType reflect.Type, args CallArgs, pipedArg *reflect.Value) ([]reflect.Value, errors.Error) {
	...
	invalidNumOfArgsError := errors.New().
		WithReason(errors.InvalidNumberOfArgumentsReason).
	...
	if !args.HasPipeSlot && pipedArg != nil {
		in := fnType.In(slot)
		if !(*pipedArg).IsValid() {
			return nil, errors.InvalidValueErr.
	...
	invalidArgError := errors.InvalidValueErr.
...
// element.
func resolveIndex(v, index reflect.Value, indexAsStr string, lax bool) (reflect.Value, errors.Error) {
	if !v.IsValid() {
		if lax {
			return reflect.Value{}, nil
		}
	...
	if v.Kind() == reflect.Interface && isNil {
		...
		return reflect.Value{}, errors.InvalidValueErr.
	...
	case reflect.Struct:
		if !indexIsStr {
			return reflect.Value{}, errors.InvalidIndexErr.
...
func indexArg(index reflect.Value, cap int) (int, errors.Error) {
	var x int64
	switch index.Kind() {
	...
	case reflect.Invalid:
		return 0, errors.InvalidIndexErr.WithMessage("cannot index slice/array/string with nil")
	default:
		return 0, errors.InvalidIndexErr.
			WithMessage(fmt.Sprintf("cannot index slice/array/string with type %s", getTypeString(index)))
	}
	if int(x) < 0 || int(x) >= cap {
		return 0, errors.InvalidIndexErr.WithMessage(fmt.Sprintf("index out of range: %d", x))
	}
	return int(x), nil
}
